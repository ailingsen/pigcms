...
<span class="okp">会员价：<em id="vsprice">￥<?php if ($product['vprice'] > 0) {echo $product['vprice'];} else {echo $product['price'];} ?></em></span>
...
<?php if(is_array($productDetail)): $i = 0; $__LIST__ = $productDetail;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$pro): $mod = ($i % 2 );++$i;?><input type="hidden" id="color_<?php echo ($pro["color"]); ?>_norms_<?php echo ($pro["format"]); ?>" value="<?php echo ($pro["num"]); ?>" did="<?php echo ($pro["id"]); ?>" price="<?php echo ($pro["price"]); ?>" vprice="<?php echo ($pro["vprice"]); ?>" class="hidden"/><?php endforeach; endif; else: echo "" ;endif; ?>
<div class="i-row">
<div class="t">数量：</div>
<div class="c buynum">
	...
	$(".color").click(function(){
		if ($(this).attr('class') != 'color on') {
			...
			if ($("#" + id + "_" + nextid).val() != null && $("#" + id + "_" + nextid).val() != '') {
				...
				$("#vsprice").text('￥' + $("#" + id + "_" + nextid).attr('vprice'));
			} else {
				$("#stock").text(0);
			}
		} else {
	...
	$(".norms").click(function(){
		if ($(this).attr('class') != 'norms on') {
			...
			if ($("#" + nextid + "_" + id).val() != '' && $("#" + nextid + "_" + id).val() != null) {
				...
				$("#vsprice").text('￥' + $("#" + nextid + "_" + id).attr('vprice'));
			} else {
				$("#stock").text(0);
			}
		} else {
